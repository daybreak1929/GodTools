...
internal class StatusEffectGroupElement : MonoBehaviour
{
    ...
    public void add_status(StatusEffect status, StatusEffectButton prefab)
    {
        ...
        if (string.IsNullOrEmpty(status.path_icon)) status.path_icon = "ui/icons/aboutthis";
...
internal class WindowStatusEffectEditor : MonoBehaviour
{
    ...
    public static void init(WindowCreatureInfo creature_window)
    {
        ...
        var scroll_window = Windows.CreateNewWindow(C.status_effect_editor_id, "人物状态编辑器");
        ...
        instance.display_status_button = Instantiate(creature_window.prefabStatus, content_transform);
        instance.display_status_button.button.onClick.AddListener(() =>
        {
            ...
            instance.reload_statuses();
        });
        instance.prefab_status_button = Instantiate(creature_window.prefabStatus);
        var SelectedIcon = new GameObject("SelectedIcon", typeof(Image));
        ...
        var statuses_title = new GameObject("Title", typeof(LocalizedText), typeof(Text));
        statuses_title.transform.SetParent(Statuses_Background.transform);
        statuses_title.GetComponent<LocalizedText>().key = "statuses";
        var statuses_title_text = statuses_title.GetComponent<Text>();
        ...
        var statuses_grid = new GameObject("Grid", typeof(GridLayoutGroup), typeof(FlexibleOneRowGrid));
        ...
        var statuses_grid_layout_group = statuses_grid.GetComponent<GridLayoutGroup>();
        ...
        var basic_statuses_text = new GameObject("Basic Statuses Title", typeof(Text)).GetComponent<Text>();
        ...
        var advance_statuses_text = new GameObject("Advance Statuses Title", typeof(Text)).GetComponent<Text>();
        ...
        var basic_statuses_rect = basic_statuses.GetComponent<RectTransform>();
        var advance_statuses_rect = advance_statuses.GetComponent<RectTransform>();


        foreach (var status in AssetManager.status.list)
        ...
        content_transform.GetComponent<RectTransform>().SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical,
            197 + (instance.basic_statuses.statuses.Count - 1) / 6 * 28 +
            (instance.advance_statuses.statuses.Count - 1) / 6 * 28);
        ...
        basic_statuses_rect.sizeDelta =
            new Vector2(191.33f, 30 + (instance.basic_statuses.statuses.Count - 1) / 6 * 28);
        ...
        var slider_bg_rect = slider_background.GetComponent<RectTransform>();
        var slider_fill_rect = slider_fill.GetComponent<RectTransform>();
        var slider_handle_rect = slider_handle.GetComponent<RectTransform>();
        effect_time_slider.transform.localScale = new Vector3(1, 1);
        effect_time_slider.transform.localPosition = new Vector3(122f, -60.5f);
        var slider_size = new Vector2(135, 15);
        ...
        var slider_val_text = slider_val_shower.GetComponent<Text>();
    ...
    private static void add_entry_button(WindowCreatureInfo creature_window)
    {
        ...
        entry.transform.Find("Button Trait/Icon").GetComponent<Image>().sprite =
            Resources.Load<Sprite>("ui/icons/iconMiracleBorn");
        var button = entry.transform.Find("Button Trait").GetComponent<Button>();
        button.onClick.RemoveAllListeners();
        button.onClick = new Button.ButtonClickedEvent();
        button.onClick.AddListener(() => { ScrollWindow.showWindow(C.status_effect_editor_id); });
    ...
    {
        var count = curr_statuses_transform.childCount;
        while (count > 0)
        {
            DestroyImmediate(curr_statuses_transform.GetChild(curr_statuses_transform.childCount - 1).gameObject);
            count--;
        }
        ...
        foreach (var button in StatusEffectGroupElement.all_buttons)
            ...
            {
                var image = button.transform.Find("SelectedIcon").GetComponent<Image>();
                image.enabled = true;
